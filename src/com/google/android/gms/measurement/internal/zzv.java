  ...
  extends zzz
{
  ...
  final Object zzaXO = new Object();
  final Semaphore zzaXP = new Semaphore(2);
  volatile boolean zzaXQ;
  ...
  private void zza(FutureTask<?> paramFutureTask)
  {
    synchronized (zzaXO)
    {
      ...
      if (zzaXI == null)
      {
        zzaXI = new zzc("Measurement Worker", zzaXK);
    ...
    throws IllegalStateException
  {
    zzjv();
    zzx.zzz(???);
    zza localzza = new zza(???, "Task exception on network thread");
    synchronized (zzaXO)
    {
      zzaXL.add(localzza);
      if (zzaXJ == null)
      {
        zzaXJ = new zzc("Measurement Network", zzaXL);
    ...
    extends FutureTask<V>
  {
    ...
    {
      zzAo().zzaWC.zzj(zzaXR, paramThrowable);
    ...
    extends Thread
  {
    ...
    {
      zzAo().zzaWF.zzj(getName() + " was interrupted", paramInterruptedException);
    }

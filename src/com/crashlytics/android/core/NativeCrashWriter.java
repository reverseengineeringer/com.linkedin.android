...
final class NativeCrashWriter
{
  ...
  {
    CustomAttributeMessage[] arrayOfCustomAttributeMessage = new CustomAttributeMessage[paramArrayOfCustomAttributeData.length];
    int i = 0;
    ...
    throws IOException
  {
    ...
    {
      localObject1 = signal;
      localObject2 = new SignalMessage((SignalData)localObject1);
      localObject3 = threads;
      if (localObject3 == null) {
        break label100;
      }
    }
    label100:
    for (Object localObject1 = new ThreadMessage[localObject3.length];; localObject1 = EMPTY_THREAD_MESSAGES)
    {
      i = 0;
      while (i < localObject1.length)
      {
        localObject4 = localObject3[i];
        localObject1[i] = new ThreadMessage((ThreadData)localObject4, createFramesMessage(frames));
        i += 1;
      }
      localObject1 = DEFAULT_SIGNAL;
      break;
    }
    Object localObject3 = new RepeatedMessage((ProtobufMessage[])localObject1);
    Object localObject4 = binaryImages;
    if (localObject4 != null) {}
    for (localObject1 = new BinaryImageMessage[localObject4.length];; localObject1 = EMPTY_BINARY_IMAGE_MESSAGES)
    {
      i = 0;
      while (i < localObject1.length)
      {
        localObject1[i] = new BinaryImageMessage(localObject4[i]);
        i += 1;
      }
    }
    localObject1 = new ExecutionMessage((SignalMessage)localObject2, (RepeatedMessage)localObject3, new RepeatedMessage((ProtobufMessage[])localObject1));
    Object localObject2 = customAttributes;
    ...
    localObject2 = new CustomAttributeData[paramMap.length];
    int i = 0;
    while (i < localObject2.length)
    {
      localObject2[i] = new CustomAttributeData((String)paramMap[i].getKey(), (String)paramMap[i].getValue());
      i += 1;
    }
    paramMap = new ApplicationMessage((ExecutionMessage)localObject1, createCustomAttributesMessage((CustomAttributeData[])localObject2));
    localObject1 = deviceData;
    localObject1 = new DeviceMessage(batteryCapacity / 100.0F, batteryVelocity, proximity, orientation, totalPhysicalMemory - availablePhysicalMemory, totalInternalStorage - availableInternalStorage);
    localObject2 = paramLogFileManager.getByteStringForLog();
    if (localObject2 == null) {
      Fabric.getLogger().d("CrashlyticsCore", "No log data to include with this event.");
    }
    paramLogFileManager.clearLog();
    if (localObject2 != null) {}
    for (paramLogFileManager = new LogMessage((ByteString)localObject2);; paramLogFileManager = new NullMessage())
    {
      new EventMessage(timestamp, "ndk-crash", new ProtobufMessage[] { paramMap, localObject1, paramLogFileManager }).write(paramCodedOutputStream);
      return;
    }
  }
